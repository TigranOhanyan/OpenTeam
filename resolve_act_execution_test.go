...
func Test_ActExecutionResolver_should_resolve_act_execution_if_all_tool_calls_have_been_resolved(t *testing.T) {
	...
	defer teamDb.Close()
	err = makeTeamForActExecutionResolverTest(ctx, teamDb)
	assert.NoError(t, err)
...
// AGENTDO create a team and get the team in a situation when a tool call is required
...
func makeTeamForActExecutionResolverTest(ctx context.Context, teamDb *TeamDb) (err error) {
	q := teamDb.Queries
