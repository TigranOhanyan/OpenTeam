...
func Test_ReactExecutionResolver_should_create_reason_for_a_blank_react_loop(t *testing.T) {
	...
	defer teamDb.Close()
	err = makeTeamForReactExecutionResolverTest(ctx, teamDb)
	assert.NoError(t, err)
...
// AGENTDO create a team and get the team in a situation when a reason or act is required
...
func makeTeamForReactExecutionResolverTest(ctx context.Context, teamDb *TeamDb) (err error) {
	q := teamDb.Queries
