...
func Test_MentionResolver_should_resolve_mention_if_all_tasks_are_resolved(t *testing.T) {
	...
	teamDb, err := teamDbFactory.NewTeamDb(ctx, "MentionResolver_should_provide_tool_result.db", testLogger)
	assert.NoError(t, err)
	assert.NotNil(t, teamDb)
	defer teamDb.Close()
	err = makeTeamForMentionResolverTest(ctx, teamDb)
	assert.NoError(t, err)
...
// AGENTDO create a team and get the team in a situation when a mention is required
...
func makeTeamForMentionResolverTest(ctx context.Context, teamDb *TeamDb) (err error) {
	q := teamDb.Queries
