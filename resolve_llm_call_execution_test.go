...
func Test_ResolveLLMCallBulkExecution_should_provide_llm_call_bulk_response(t *testing.T) {
	...
	defer teamDb.Close()
	err = makeTeamForResolveLLMCallExecutionTest(ctx, teamDb)
	assert.NoError(t, err)
...
// AGENTDO create a team and get the team in a situation when a llm call bulk response is required
...
func makeTeamForResolveLLMCallExecutionTest(ctx context.Context, teamDb *TeamDb) (err error) {
	q := teamDb.Queries
