...
This engine acts as the "Operating System" for the team. It is an orchestrator that reads the state, decides who needs to act, and coordinates the flow of information, supporting both sequential tasks and parallel execution.

### How the Engine Works
There is one active orchestrator per request. It acts as the scheduler for the organization.

1.  **Read State:** The engine queries the SQLite database to find all pending actions (e.g., new messages in Channels, pending Tasks, or unresolved tool calls).
...
3.  **Execute Tasks (Parallel & Sequential):** 
...
6.  **Yield:** Once all Members have finished their Tasks and no further tasks are pending, the engine terminates and returns the final state.
...
3.  **Auditability:** Every step of the engine (every Task shift, every tool call) is written to the SQLite file. The resulting file is a perfect, linear audit log of the entire organizational thought process.