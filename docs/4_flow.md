...
*   *Scope:* They return data directly back to the agent's current Task. They **do not** change the state of the conversation or route messages to other participants.
...
When a new message arrives in a Channel, the framework wakes up the relevant Member. The data flows through that Member's assigned Tasks sequentially.
...
*   The framework receives the Task-Pass call and loads the next Task.
