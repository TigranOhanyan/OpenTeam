...
The implementation architecture specifies that the engine executes tasks and tools in parallel. While SQLite is highly capable, concurrent writes (e.g., multiple agents writing messages or tool results to the same `.sqlite` file at the exact same millisecond) can lead to `database is locked` errors.
