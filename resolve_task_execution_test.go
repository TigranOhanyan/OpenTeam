...
func Test_TaskExecutionResolver_should_create_react_loop_for_a_blank_task(t *testing.T) {
	...
	defer teamDb.Close()
	err = makeTeamForTaskExecutionResolverTest(ctx, teamDb)
	assert.NoError(t, err)
...
// AGENTDO create a team and get the team in a situation when there is a need to close the task
...
func makeTeamForTaskExecutionResolverTest(ctx context.Context, teamDb *TeamDb) (err error) {
	q := teamDb.Queries
